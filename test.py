...
class TestCflow2Dot(unittest.TestCase):
    ...
    def test_build_excludes(self):
        ap = get_parser()
        opts = ap.parse_args("-e foo --no-stdlib tests/a.c".split(" "))
        exclude = build_excludes(opts)
        self.assertEqual(True, "foo" in exclude)

    def test_exclude(self):
        ap = get_parser()
        opts = ap.parse_args("-e foo --no-stdlib tests/b.c".split(" "))
        flow = call_cflow(opts.cflow_args)
        res = get_output(opts, flow)
        self.assertEqual(['digraph G {\nnode [peripheries=2 style="filled,rounded" fontname="Vera Sans YuanTi Mono" color="#eecc80"];\nrankdir=LR;\nlabel="tests/b.c"\n', 'main [shape=box];\n', '}\n'], res)
